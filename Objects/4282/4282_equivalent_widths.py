...
mast_folder=source / "Medium_Resolution/Spectra1D/MAST"
jades_folder=source / "Medium_Resolution/Spectra1D/JADES"


#Extract data with Pandas
df = pd.read_csv( source / "list_of_candidates.tsv", sep='\t')
ID_array = df["NIRSpec_ID"]
JADES_files = df["JADES_FILENAME_G395M"]
MAST_files = df["MAST_FILENAME_G395M"]
z_array = df["redshift"]
...
Ha_ini, Ha_end = 6568 *u.AA, 6590 *u.AA
Hb_ini, Hb_end = 4850 *u.AA, 4870 *u.AA
O_ini, O_end = 4922 *u.AA, 5031 *u.AA
#//////////////////////////////////////
...
def compute_EWs(lambda_rest, flux):
  ...
  Ha_region_excl = SpectralRegion(Ha_ini, Ha_end + 15*u.AA )
  Ha_right = SpectralRegion(Ha_end + 500 *u.AA, lamb[-1])
