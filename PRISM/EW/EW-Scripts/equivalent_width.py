...
def main():
  ...
  EWD.to_csv(f'{output_folder}/EW_output_aug.tsv', sep="\t", index=False, mode='a', header=True)
...
def compute_EWs(lambda_rest, flux, lines_lim):
  ...
  flux_err_Ha = flux_stdDev(lambda_rest, flux, Ha_ini.value - 1000, Ha_ini.value, Ha_end.value, Ha_end.value + 1000 )
  ...
  Ha_right = SpectralRegion(Ha_end + 800 *u.AA, lamb[-1])
  ...
  #Uncertainty of the flux
  flux_err_Hb = flux_stdDev(lambda_rest, flux, Hb_ini.value - 800, Hb_ini.value, O_end.value, O_end.value + 800)
  flux_err_Jy = flux_err_Hb * np.ones(len(flux)) * u.Jy
  flux_uncertainty = StdDevUncertainty(flux_err_Jy)

  #For Hb and [OIII]
  Hb_left = SpectralRegion(lamb[0], Hb_ini - 800 *u.AA)
  Hb_region = SpectralRegion(Hb_ini, Hb_end)
  O3_region = SpectralRegion(O_ini, O_end)
  O3_right = SpectralRegion(O_end + 1200 *u.AA, lamb[-1])
...
def EquivalentWidths(mast_file, jades_file, z, ID, lines_limits):

  try:
    with fits.open(jades_file) as j_f:
      specdata = j_f[1].data

      # JADES flux is in erg cm^-2 s^-1 AA^-1; convert it to Jy.
      flux_erg = specdata['FLUX']
      ...
      jades_EW = compute_EWs(lambda_rest, flux, lines_limits)

  except FileNotFoundError:
    print(f'JADES file for Object {ID} not found')
    jades_EW = (np.nan,) * 6
...
def save_EW(index):
  ...
  mast_output, jades_output = EquivalentWidths(
      mast_file, jades_file, z, ID, lines_limits
  )
  print(mast_output, jades_output)
