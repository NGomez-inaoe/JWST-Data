...
def save_EW(index):
    ...
    lines_df = pd.read_csv(f'{output_folder}/EW_output_v6_1.tsv', sep='\t')

    Ha_ini = lines_df["Ha_ini"][index] *u.AA
    Ha_end = lines_df["Ha_end"][index] *u.AA
    Hb_ini = lines_df["Hb_ini"][index] *u.AA
    Hb_end = lines_df["Hb_end"][index] *u.AA
    O_ini = lines_df["[OIII]_ini"][index] *u.AA
    O_end = lines_df["[OIII]_end"][index] *u.AA

    lines_limits = [Ha_ini, Ha_end, Hb_ini, Hb_end, O_ini, O_end]

    mast_output, jades_output = EquivalentWidths(mast_file, jades_file, z, ID, lines_limits)
