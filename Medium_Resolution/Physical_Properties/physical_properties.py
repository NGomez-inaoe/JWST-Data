...
#Data File with the flux data
df = pd.read_csv("/home/nicolas/Documents/Research/PhD/JWST-Data/Medium_Resolution/Output_data/fluxes_for_abundance_all_filters_dline_3.tsv", sep='\t')

#Pyneb preliminarities
Wave = int
...
def main():
    ...
    #Single object computation
    index = 1
    compute_for_single_obj(df, index, 'MAST')

    #save_for_many(df, 'JADES')


#========================================== Functions =================================#

def save_for_many(df=df, source='JADES'):

    for index in range(4):
        dataFrame = flux_from_df(df, index, source, print_selection= False)
        
        flux = dataFrame['flux']
        flux_error = dataFrame['error']
...
def compute_for_single_obj(df, index, source='JADES', print_selection=False):
    ...
    print(f"Results for Object: {df['ID'].iloc[index]}")
    print_pyneb_uncertainty_result(result)

#-----------------------------------------------------------------------------------------------


def flux_from_df(df, index, source="MAST", *, return_details = False, print_selection = False):

    """
    Select the highest S/N measurement for each line among G235M, G395M and G395H.

    Returns a dict with:
      - "flux": selected fluxes
      - "error": selected errors
      - "filter": filter used for the selection
      - "snr": selected S/N
    """
    line_label_map = {
        3727: "[OII] 3727",
    ...
    if isinstance(index, (int, np.integer)):
        row = df.iloc[index]
    elif index in df.index:
        row = df.loc[index]
    elif "ID" in df.columns and index in df["ID"].values:
        row = df[df["ID"] == index].iloc[0]
    else:
        raise KeyError(f"Row {index!r} not found in dataframe index or ID column.")
    ...
    for wave, label in line_label_map.items():
        best = None
        for filt in filters:
            flux_col = f"Flux ({label}) {source} {filt}"
            err_col = f"F_err({label}) {source} {filt}"
            if flux_col not in df.columns or err_col not in df.columns:
                continue

            flux_val = row[flux_col]
            err_val = row[err_col]
            if pd.isna(flux_val) or pd.isna(err_val) or err_val == 0:
                continue

            snr = float(flux_val) / abs(float(err_val))
            candidate = {
                "flux": float(flux_val),
                "error": float(err_val),
                "filter": filt,
        ...
        if best is None:
            selected_flux[wave] = np.nan
            selected_error[wave] = np.nan
            selected_filter[wave] = None
            selected_snr[wave] = None
        else:
            selected_flux[wave] = best["flux"]
            selected_error[wave] = best["error"]
            selected_filter[wave] = best["filter"]
            selected_snr[wave] = best["snr"]

    if print_selection:
        print(f"Selected measurements for row {index}:")
        for wave in sorted(line_label_map):
            print(
                f"{wave}: flux={selected_flux[wave]}, "
                f"err={selected_error[wave]}, "
                f"filter={selected_filter[wave]}, "
                f"S/N={selected_snr[wave]}"
            )

    result = {"flux": selected_flux, "error": selected_error}
    if return_details:
        result["filter"] = selected_filter
        result["snr"] = selected_snr
